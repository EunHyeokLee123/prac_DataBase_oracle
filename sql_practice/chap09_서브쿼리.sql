...
-- employee_id가 103번인 사람의 job_id가 동일한 job_id를 가진 사람을 조회
...
job_id
FROM employees
WHERE last_name = 'King')  --> 서브쿼리의 결과가 하나이상 출력되서 에러 발생
;

-- 다중 행 연산자 (IN, ANY, EXIST)

-- IN : 조회된 목록의 어떤 값과 같은 지를 비교
-- ex) salary IN (200, 300, 400) => salary가 세 값중 하나인지
-- true, false를 리턴
...
-- ANY (SOME도 동일) : 값을 서브쿼리에 의해 리턴된 각각의 값과 비교해서
...
-- EXISTS : 서브쿼리가 하나 이상의 행을 리턴하면 참으로 간주.
...
SELECT
e1.first_name, e1.salary, e2.부서급여평균
FROM employees e1
LEFT JOIN (
SELECT
department_id, TRUNC(AVG(salary),0) AS 부서급여평균
FROM employees
...
-- 부서별 최고 급여를 받는 직원을 조회해주세요
-- 사원이름first, 급여. 부서이름

SELECT
e.first_name, tb.최고, d.department_name
FROM employees e
JOIN
(SELECT
    department_id, 
...
ON e.department_id = tb.department_id
AND e.salary = tb.최고
JOIN
departments d
